...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Aggregate image-level multiclass task CSV into a patient-level task CSV."
    )
...
def main() -> None:
    ...
    class_counts = patient_df[args.label].value_counts().sort_index()
    print(f"Saved patient-level CSV: rows={n_patients} -> {args.output_csv}")
    for cls in TIRADS_CLASSES:
        cnt = int(class_counts.get(cls, 0))
        if cnt > 0:
            print(f"  TIRADS {cls}: {cnt}")
